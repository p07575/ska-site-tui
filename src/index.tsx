...
// ── Startup diagnostics: print env vars and feature status ───────────
const NOT_SET = "(not set)";
const envStatus = {
  HALO_BASE_URL: process.env.HALO_BASE_URL ?? NOT_SET,
  HINDSIGHT_API_URL: process.env.HINDSIGHT_API_URL ?? NOT_SET,
  AI_BASE_URL: process.env.AI_BASE_URL ?? NOT_SET,
  AI_API_KEY: process.env.AI_API_KEY ? "****" : NOT_SET,
  AI_MODEL: process.env.AI_MODEL ?? NOT_SET,
  DEFAULT_LOCALE: process.env.DEFAULT_LOCALE ?? "en",
};
const aiChatEnabled = isAiEnabled();
const memoryEnabled = envStatus.HINDSIGHT_API_URL !== NOT_SET;

console.log("\n── SKA-SITE startup diagnostics ────────────");
for (const [key, val] of Object.entries(envStatus)) {
  console.log(`  ${key.padEnd(20)} ${val}`);
}
console.log("--------------------------------------------");
console.log(`  AI Chat ........... ${aiChatEnabled ? "enabled" : "disabled"}`);
console.log(`  Memory system ..... ${memoryEnabled ? "enabled" : "disabled"}`);
console.log("────────────────────────────────────────────\n");
...
try {
  ...
  if (err.code === "EADDRINUSE") {
    console.error(`  Port ${PORT} is already in use. Stop the process using it and retry.`);
  }
  process.exit(1);
}
